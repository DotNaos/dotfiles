...
- `os-work` (including its WSL environment) and `os-yoga` / `os-yoga-unix` are different OS installations on the same physical work device. Neither installation may connect to `os-vps`, directly or through a proxy, tunnel, relay, or personal machine. All work-memory processing, storage, indexes, and backups must remain on that work device. Do not provision either installation as a personal-memory client. Remote access to this device is not authorized; do not attempt SSH or another remote-access method without a new explicit user instruction.
